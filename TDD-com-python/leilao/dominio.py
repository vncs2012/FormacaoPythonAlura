...
class Leilao:
    ...
    def _user_diferentes(self, lance):
        if self.__lances[-1].usuario != lance.usuario:
            return True
        raise LanceInvalido('Não pode 2 lances seguidos')

    def _valor_maior_que_lance_anterior(self, lance):
        if lance.valor > self.__lances[-1].valor:
            return True
        raise LanceInvalido(
            'O valor do lance deve ser maior que lance anterior')

    def _lance_valido(self, lance):
        return not self._tem_lances() or (self._user_diferentes(lance) and
